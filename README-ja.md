...
4. 表現上の盲点は避けるべきです。commonmarkでは、`a*?*b` と書くとフランキングルール [^1] によって最初のアスタリスクが右フランキングとして分類されるため、HTML `a<em>?</em>b` を生成したい場合、運が悪かったとしか言いようがない。これを回避する方法はありますが醜いです（ `a` の代わりに数値参照を使用します。）。djotでは、この種の表現上の死角があってはなりません。
