...
4. 表現上の盲点は避けるべきです。`a<em>?</em>b` commonmarkでは、 `a*?*b` フランキングルール [^1] によって最初のアスタリスクが右フランキングとして分類されるため、HTMLを生成する場合はアンラッキーです。これを回避する方法はありますが醜いです（ `a` の代わりに数値参照を使用します)。djotでは、この種の表現上の死角があってはなりません。
