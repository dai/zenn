...
4. 表現上の盲点は避けるべきです。commonmarkでは、`a<em>?</em>b` 、 `a*?*b` フランキングルール [^1] によって最初のアスタリスクが右フランキングとして分類されるため、HTMLを生成する場合、運が悪かったとしか言いようがない。これを回避する方法はありますが醜いです（ `a` の代わりに数値参照を使用します)。djotでは、この種の表現上の死角があってはなりません。

[^1]: いわゆる分かち書き。左フランキングで開いて右フランキングで閉じてはじめて強調されるというルールです。
